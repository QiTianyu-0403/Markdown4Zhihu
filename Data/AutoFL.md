...
FedAvg被认为是事实上的FL算法。FedAvg使用小批量大小为$B$的随机梯度下降(SGD)在$K$个选定设备上训练$E$个epoch的模型，其中$K$是参与FL的$N$个设备的小子集。（这里和上一篇FL开山文章的字母定义不同）然后$K$个设备将各自的模型梯度上传到云。

关键问题就是：（1）选取哪些个$K$个设备进行聚合；（2）***在每个子设备的训练执行目标需要达到什么效果。***
...
本文提出了一个基于学习的能量优化框架AutoFL，该框架选择$K$个参与者和执行目标来保证模型质量，同时**最大化单个参与者（或所有参与者的集群）的能量效率**。由于最优决策随神经网络特性、FL全局参数、参与设备的轮廓、局部训练样本的分布以及随机运行时间方差而变化，因此设计空间庞大且无法枚举。因此，我们设计了一种**强化学习技术**。
...
> <img src="AutoFL/image-20211202113332255.png" alt="image-20211202113332255" style="zoom:50%;" />
>
> 涉及参数：总设备$N$，每次选择训练的设备个数$K$，本地设备选择训练的部分数据$B$，本地训练轮数$E$
>
> 训练步骤：
>
> - 服务器从$N$个设备中选择$K$个参与者；
> - 将全局模型广播到所选择的设备；
> - 每个参与者使用批大小为$B$的本地数据样本在$E$个epoch上独立地训练模型；
...
> <img src="AutoFL/image-20211202114604789.png" alt="image-20211202114604789" style="zoom:50%;" />
>
...
<img src="AutoFL/image-20211205153011659.png" alt="image-20211205153011659" style="zoom:50%;" />
...
>
> <img src="AutoFL/image-20211205154238813.png" alt="image-20211205154238813" style="zoom:40%;" />
>
> <img src="AutoFL/image-20211205154257441.png" alt="image-20211205154257441" style="zoom:40%;" />
...
> <img src="AutoFL/image-20211205162341859.png" alt="image-20211205162341859" style="zoom:50%;" />
>
> - （a）：无干扰和网络信号稳定
> - （b）：存在设备上干扰时
> - （c）：网络信号强度较弱时
...
<img src="AutoFL/image-20211205171445831.png" alt="image-20211205171445831" style="zoom:50%;" />
...
> ![image-20211206085634119](AutoFL/image-20211206085634119.png)
>
...
如果探索所有可能动作，RL收敛时间会过长，故采用$\epsilon$-贪婪算法。
...
![image-20211206095732824](AutoFL/image-20211206095732824.png)

$S_{CONV}$、$S_{FC}$、$S_{RC}$用来表示CONV、FC、RC层的数量；

$S_B$、$S_E$、$S_K$表示批次、训练次数、数据划分；

$S_{Co-CPU}$、$S_{Co-MEM}$表示CPU使用率和内存使用率；

$S_{Network}$表示网络不稳定性建模；

$S_{Data}$表示每个设备在聚合中拥有的数据类别数。
...
$R_{energy-local}$：每个单独设备的估计能耗；

$R_{energy-global}$：所有设备的能耗估计值；

$R_{accuracy}$：NN的测试精度。

需要先计算**“计算能量”**$E_{comp}$，如下，其中$E_{core}^i$是第$i$个核所消耗能量，$t_{busy}^f$是频率$f$下繁忙状态，$P_{busy}^f$是频率$f$下消耗能量，$t_{idle}$是空闲时间。
$$
E_{\text {comp }}=\sum_{i} E_{c o r e}^{i}
$$

$$
E_{\text {core }}=\sum_{f}\left(P_{b u s y}^{f} \times t_{b u s y}^{f}\right)+P_{i d l e} \times t_{i d l e}
$$

对于GPU来说，其计算能量为：
$$
E_{\text {comp }}=\sum_{f}\left(P_{\text {busy }}^{f} \times t_{\text {busy }}^{f}\right)+P_{\text {idle }} \times t_{\text {idle }}
$$
计算完计算能量后。计算其**“通讯能量”**，其中$t_{TX}$是发送梯度更新时测量的等待时间，$P_{TX}^S$在信号强度为$S$的情况下，设备在$t_{TX}$期间消耗的功率：
$$
E_{\text {comm }}=P_{T X}^{S} \times t_{T X}
$$
同时，还计算非选定设备的**“空闲能量”**，其中$t_{round}$是训练轮次消费的时间：
$$
E_{i d l e}=P_{i d l e} \times t_{\text {round }}
$$
则每一台设备的$R_{energy-local}$的计算如下，其中$S_t$是一组选定的设备：

<img src="AutoFL/image-20211206142911568.png" alt="image-20211206142911568" style="zoom:40%;" />

则$R_{energy-global}$的计算为：
$$
R_{e n e r g y_{-}{g l o b a l}}=\sum_{i}^{N} R_{e n e r g y_{-} l o c a l}
$$
对于$R_{accuracy}$其计算如下，其中$R_{accuracy-prev}$是上一轮训练神经网络的测试精度：

<img src="AutoFL/image-20211206144803457.png" alt="image-20211206144803457" style="zoom:40%;" />

如果选择的动作不能提高上一轮的准确性，奖励是$R_{accuracy}-100$（即准确度离100%有多远），以避免选择下一轮推理的动作。否则，将根据全局能量、局部能量、准确度和精确度提升量来计算每个设备的奖励。
...
<img src="AutoFL/image-20211206161158304.png" alt="image-20211206161158304" style="zoom:40%;" />

<img src="AutoFL/image-20211206161222577.png" alt="image-20211206161222577" style="zoom:40%;" />

> - 随机值初始化Q表
> - 如果随机值小于ϵ，则AutoFL随机选择参与者并确定A进行探索。否则，它将按Q对设备进行排序，并选择前K个设备。
> - 为每个选定的参与者选择具有最大Q的A
> - 每轮本地训练聚合结束后，计算更新$R_{energy-local}$、$R_{energy-global}$、$R_{accuracy}$
> - 观察新的状态，利用$Q^\prime$选择相应的参与者和执行目标，再利用$\mu$和$\gamma$更新Q表。
...
有两个超参数需要学习，学习率$\gamma$和折扣率$\mu$，经过对比实验，取学习率为0.9，折扣率为0.1
...
<img src="AutoFL/image-20211207142704099.png" alt="image-20211207142704099" style="zoom:50%;" />
...
<img src="AutoFL/image-20211207150128971.png" alt="image-20211207150128971" style="zoom:50%;" />

AutoFL试图准确地预测参与者设备的近最佳集群，而不考虑全局参数。因此，在能源效率和收敛时间方面，它总是超过FedAvg-Random、Performance和Power的基线设置。

- ***随机方差的适应性：***

<img src="AutoFL/image-20211207153225779.png" alt="image-20211207153225779" style="zoom:50%;" />
...
<img src="AutoFL/image-20211207154539022.png" alt="image-20211207154539022" style="zoom:50%;" />

AutoFL动态学习**数据异质性对收敛时间和能量效率的影响**，适应不同设备之间不同的数据异质性水平。因此，即使在存在数据异质性的情况下，它也能达到接近最优的能源效率、收敛时间和模型质量。

- ***预测准确性：***

<img src="AutoFL/image-20211207155047769.png" alt="image-20211207155047769" style="zoom:50%;" />
...
<img src="AutoFL/image-20211207160405448.png" alt="image-20211207160405448" style="zoom:50%;" />

<img src="AutoFL/image-20211207160419754.png" alt="image-20211207160419754" style="zoom:50%;" />
...
<img src="AutoFL/image-20211207163026403.png" alt="image-20211207163026403" style="zoom:50%;" />
